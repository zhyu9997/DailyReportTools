...
Tests/DailyReportTests/             # Swift Testing 套件，193 tests / 13 suites（详见 14.测试）
```
...
`Tests/DailyReportTests/` 下用 Swift Testing 框架，193 tests / 13 suites 全绿。运行需 Xcode 工具链（纯 CLT 不带 Testing 模块）：
...
| Suite | 用例数 | 覆盖点 |
|---|---|---|
| `AppStoreTests` | 35 | Tag/DailyReport/TodoItem/WorkEntry/Meeting/Review 的 CRUD + 关系重建（4 张中间表）+ CASCADE + transactional 回滚 + unknown id 静默 no-op（update + delete 全覆盖，R27-F 补 deleteTag/Todo/Meeting）+ addReview FK 违规 + addReview order 事务内 MAX+1（R23-A）+ markEntryDone race 防御 + vacuum + insert 路径的 tag/review 同步绑定（R21-A） |
