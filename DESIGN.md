...
5. **今日会议**（仅 `filteredMeetings` 非空时显示）
   - `todayMeetingRow`：紫色主题，主题 + 周期胶囊 + 概要 + 标签 chips
...
- 列头：图标 + 文字 + 计数胶囊
- 卡片列表：
  - `.planned` → `plannedSections`（按优先级高/中/低分组，组头可折叠，整组作 drop 目标）
  - `.blocker` → `statusSections`（按状态进行中/观察中/已关闭分组，同上）
  - `.done` → 平铺
...
- 只读态：标题 + 优先级徽章 + 详情 + metaRow（完成日 / 计划日+周期 / 状态+求助人）+ tagRow + 编辑/删除按钮
- 编辑态：标题/详情 TextField + 标签多选 Menu + extraEditRow + 优先级 + 取消/保存
...
- 底部：「评审」按钮（展开 inlineAddReviewer）+ 「编辑」按钮
