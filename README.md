...
> Swift Testing 框架随 Xcode 提供（CLT 不带），需临时指定 `DEVELOPER_DIR`。**113 tests / 8 suites** 覆盖：

| Suite | 覆盖点 |
|---|---|
| `AppStoreTests` (22) | Tag/DailyReport/TodoItem/WorkEntry/Meeting/Review 的 CRUD + CASCADE + 关系重建 + unknown id 静默 no-op + addReview FK 违规 + markEntryDone race + transactional 回滚 + vacuum |
| `MigratorTests` (2) | v1→v2 升级路径：dedup 合并 note + 迁移 tag 关系；UNIQUE 约束生效 |
| `BackupServiceTests` (19) | weekKey + 各 prefix backup 存在性 + prune 策略 + decode 拒绝高版本/坏 JSON |
| `BackupServiceIntegrationTests` (4) | snapshotAtomic 全实体 + restore round-trip + 空 snapshot 清库 |
| `RecurrenceServiceTests` (9) | sweep 推进 + markDone 克隆下一期 + race no-op |
...
├── Database/               # GRDB 持久层：Records/Migrator(v1+v2)/AppDatabase/AppStore/RecordQueries/Environment
├── Models/                 # 纯数据/枚举（Recurrence、WorkKind 等）
├── Views/                  # 概要 / 时间线 / 会议 / 周报 / 设置 / 菜单栏面板
├── Components/             # 复用组件（标签选择、KindPicker、RecurrenceEditor、WriteErrorAlert、TagFilterMenu…）
└── Services/               # 备份 / 导出 / 周期推进 / 提醒 / 日志
Tests/DailyReportTests/      # 113 tests / 8 suites（详见 DESIGN.md §14）
